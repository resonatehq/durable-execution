...
| `pending_workflow_has_timer` | `Timers` | a pending workflow has a timer armed, no later than its root's deadline; so does every pending external promise |
