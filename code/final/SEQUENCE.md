...
*These are drawn by hand, from the code.*
...
| `handler` / `Server` | `server.py`. The HTTP entry point: `handler = serve()` in `main.py` has `config.py` build one `Server` per container at import |
| `Worker` | `worker.Worker` — **an object, not a service**. `Server.worker`, built by `config.py` beside the engine and called in-process. `run` claims the task and decides what the outcome means; `_attempt` runs the function once, from the top |
...
arrives as an HTTP request, the handler hands it to `Worker.run`, and
...
sequenceDiagram
    ...
    C->>+H: POST /, {kind, data}
    H->>H: authorized() — OIDC, for execute and timeout only
    ...
    Note over K: sweep first, then the request against<br/>the swept document, then commit:<br/>one document, one deadline
    K-->>-E: [SetTimeout?, SetDocument, DelTimeout?, Send...], reply

    alt the decision changed nothing
        Note over E,S: no effects at all: nothing is written,<br/>so a read costs no conditional write
    else a transition
        E->>T: 1. arm, if the deadline moved — create("/", {kind: timeout, origin}, not_before=at)
        T-->>E: task name, recorded in the document
        E->>S: 2. commit — put("wf/{origin}", whole document, if_match=generation)
        S-->>E: new generation
        E->>T: 3. disarm, if the deadline moved — delete(the name the predecessor armed)
        E->>T: 4. send — create(target, {kind: execute, taskId, version})
    end
...
`Worker.run`, in the protocol's own words. The function is run from the top
every time; what stops a call running twice is not memory but the promise
each call creates at its own position.
...
sequenceDiagram
    ...
    alt somebody else holds it, or it has moved on
        ...
        Fn->>E: task.fence(promise.create "research.1:2", target=BASE_URL/)
        Fn->>E: task.fence(promise.create "research.1:3", target=BASE_URL/)
        Fn->>E: task.fence(promise.create "research.1:4", target=BASE_URL/)
        E-->>Fn: pending — each create dispatches its own task
        Fn-->>-W: Blocked(["research.1:2", "research.1:3", "research.1:4"])
...
The other three ways out of `run`:
...
The only message no client is meant to send (with `ROUTES_ACCOUNT` set,
it needs the queue's token). It is the same `process`, consulting
`handle_internal` instead, and it is idempotent: a duplicate finds nothing
due, the kernel returns no effects, and nothing is written.
...
sequenceDiagram
    ...
    Note over K: settle expired promises: reject, or resolve timers<br/>run their settlement chains<br/>re-dispatch pending tasks past their retry<br/>re-pend acquired tasks past their lease
    K-->>-E: effects

    alt nothing was due
        Note over E,S: no write, no message, nothing
    else something expired
        E->>Q: arm the next deadline, if there is one
        E->>S: put(if_match) — one write for the whole sweep
        E->>Q: disarm the old deadline
        E->>Q: dispatch whatever was re-pended or woken
    end
    E-->>-H: Reply
    H-->>-Q: 200 {timeout}
```
...
owes this either a generous retry policy on the queue or something that
posts `{kind: timeout, origin}` on its own schedule, such as a Cloud
Scheduler job; nothing in this package does that for you. `test_queue.py`
demonstrates the hole and the remedy beside it.
...
sequenceDiagram
    ...
    C->>F: POST / promise.create "research.1", target=BASE_URL/
    F->>S: put(if_absent) — the run exists
    F->>Q: create(BASE_URL/, execute research.1)
    F-->>C: 200 pending
    ...
    F->>S: put — resolved
    F->>Q: create(listener address, unblock) — if anyone registered one
```
