...
class Store:
    ...
    def create(self, id: str, params: Any) -> Promise:
        return self._db.setdefault(id, Promise(id, params))

    def settle(self, id: str, state: str, value: Any) -> Promise:
        p = self._db[id]
        if p.state == PENDING:
            self._db[id] = Promise(id, p.params, state, value)
        return self._db[id]
```
...
The params are on the promise so that a process which did not make the call can
...
#                                 Promise(id, params, state, value)

s = Store()

s.create("a", 1)                # Promise("a", 1, PENDING)
s.create("a", 2)                # Promise("a", 1, PENDING)      <- 2 discarded

s.settle("a", RESOLVED, 9)      # Promise("a", 1, RESOLVED, 9)
s.settle("a", RESOLVED, 8)      # Promise("a", 1, RESOLVED, 9)  <- 8 discarded
s.settle("a", REJECTED, Err())  # Promise("a", 1, RESOLVED, 9)  <- so is Err

s.create("a", 3)                # Promise("a", 1, RESOLVED, 9)  <- replay sees
```

Nothing after the first create changes the params, and nothing after the first
...
and you get the same promise, whatever params you pass after the first — those
are of record too. This is what makes replay work: a restarted program calls
`create` on ids that already exist and gets back what happened last time.

**Settling is first-writer-wins.** `settle` moves a pending promise to
`RESOLVED` or `REJECTED`, and refuses to move a settled one anywhere. A step that runs twice cannot
...
As properties, which is how they are actually checked:
...
def test_settled_stays_settled(id, a, b):
    ...
    assert s.create(id).value == a == s.settle(id, REJECTED, b).value
```
...
>
  <StackEvolution
    columns={[
      { label: '1', frames: ['agent', 'store.create'] },
      { label: '2', frames: ['agent', 'llm(messages)'] },
      { label: '3', frames: ['agent', 'store.settle'] },
    ]}
  />
</Figure>
...
>
  <StackEvolution
    accentFrames={['invoke']}
    columns={[
      { label: '1', frames: ['agent', 'invoke'] },
      { label: '2', frames: ['agent', 'invoke', 'create promise'] },
      { label: '3', frames: ['agent', 'invoke', 'llm(messages)'] },
      { label: '4', frames: ['agent', 'invoke', 'settle promise'] },
    ]}
  />
</Figure>
