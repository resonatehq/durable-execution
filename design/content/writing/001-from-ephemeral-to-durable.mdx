...
async def agent(prompt: str, store: Store, run_id: str):
    ...
    if p.state == PENDING:
        try:
            p = store.settle(id, RESOLVED, await llm(messages))
    ...
    while "tool" in response:
        ...
        if p.state == PENDING:
            try:
                p = store.settle(id, RESOLVED, await tools[name](**args))
            except Exception as e:
                p = store.settle(id, REJECTED, e)
...
The obvious cost is that fourteen lines of agent became forty-nine. Three call
...
async def durable(store, id, func, *args):
    ...
    if p.state == PENDING:
        try:
            p = store.settle(id, RESOLVED, await func(*args))
        except Exception as e:
            p = store.settle(id, REJECTED, e)
