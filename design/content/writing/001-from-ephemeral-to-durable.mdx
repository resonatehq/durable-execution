...
async def agent(prompt: str, store: Store, run_id: str):
    ...
    while "tool" in response:
        ...
        if p.state == REJECTED:
            raise p.value
        messages.append({"role": "tool", "content": p.value})
...
The obvious cost is that fourteen lines of agent became fifty-two. Three call
sites, three copies of the same twelve lines. That part is easy to fix — factor it
out:
