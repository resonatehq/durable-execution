...
async def agent(prompt: str, store: Store, run_id: str):
    ...
    while response.tool_calls:
        for call in response.tool_calls:
            ...
            p = store.create(id, call)
            if p.state == PENDING:
                try:
                    p = store.settle(id, RESOLVED, await tool(call))
                except Exception as e:
                    p = store.settle(id, REJECTED, e)
...
The obvious cost is that thirteen lines of agent became fifty. Three call sites,
three copies of the same twelve lines. That part is easy to fix — factor it
out:
