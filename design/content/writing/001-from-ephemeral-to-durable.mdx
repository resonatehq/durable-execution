...
All three show up in one session, which is the whole specification:

```python name="Store, in one session"
s = Store()

s.create("a")           # Promise("a", "pending")
s.create("a")           # Promise("a", "pending")      <- the same promise

s.resolve("a", 1)       # Promise("a", "resolved", 1)
s.resolve("a", 2)       # Promise("a", "resolved", 1)  <- 2 is discarded
s.reject("a", Err())    # Promise("a", "resolved", 1)  <- so is Err

s.create("a")           # Promise("a", "resolved", 1)  <- and a replay sees it
```

Nothing after the first settle can change what that id means. Read down the
right-hand column and the three properties name themselves.
...
@given(ids)
def test_create_is_idempotent(id):
    s = Store()
    assert s.create(id) == s.create(id)

@given(ids, values, values)
def test_first_settle_wins(id, a, b):
    s = Store(); s.create(id)
    assert s.resolve(id, a) == s.resolve(id, b)

@given(ids, values, values)
def test_settled_stays_settled(id, a, b):
    s = Store(); s.create(id); s.resolve(id, a)
    assert s.create(id).value == a == s.reject(id, b).value
```
...
>
  <StackEvolution
    columns={[
      ...
      { label: '3', frames: ['agent', 'store.resolve'] },
    ]}
  />
</Figure>
